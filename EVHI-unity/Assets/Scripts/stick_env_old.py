...
borne_min_length, borne_max_length = 0.01, 1  # Exemple de plages pour la longueur
...
class CustomStickEnv(gym.Env):
    ...
    def __init__(self, target_performance):
        ...
        self.perffile = f'./logs_stats_droQ/performance_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.txt'
        self.file = open(self.perffile, 'w')
        self.file.close()
        
        # Create a file begining with "reward_" and ending with the date and time
        self.rewardfile = f'./logs_stats_droQ/reward_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.txt'
    ...
    def reset(self, seed=None, options=None):
        ...
        return np.array([initial_performance] * 10 + [0.1, 1] * 10).astype(np.float32), None
