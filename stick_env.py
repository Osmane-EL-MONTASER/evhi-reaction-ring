...
nombre_d_essais = 1000  # Nombre de données à générer
...
# Fonction pour simuler la performance du joueur
def simulate_performance(length, width, speed):
    # Cette fonction doit être conçue pour refléter la manière dont les paramètres affectent la performance
    # Ici, nous utilisons une fonction simplifiée pour l'exemple
    # La performance diminue avec l'augmentation de la longueur et de la vitesse et l'augmentation de la largeur
    performance = 100 - (length / borne_max_length * 10 + speed / borne_max_speed * 10 - width / borne_max_width * 10)
    performance = np.clip(performance, 0, 100)  # Assure que la performance est entre 0 et 100
    return performance
